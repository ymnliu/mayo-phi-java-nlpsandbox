...
import org.example.PersonNameAnnotator;
...
public interface TextPersonNameAnnotationsApi {
    ...
    default ResponseEntity<TextPersonNameAnnotationResponse> createTextPersonNameAnnotations(@ApiParam(value = ""  )  @Valid @RequestBody(required = false) TextPersonNameAnnotationRequest textPersonNameAnnotationRequest) {
        String text = textPersonNameAnnotationRequest.getNote().getText();
        List<TextPersonNameAnnotation> annotations = new PersonNameAnnotator()
            .annotate(text);
        TextPersonNameAnnotationResponse res = new TextPersonNameAnnotationResponse()
            .textPersonNameAnnotations(annotations);

        return new ResponseEntity<TextPersonNameAnnotationResponse>(res, HttpStatus.OK);
    }
